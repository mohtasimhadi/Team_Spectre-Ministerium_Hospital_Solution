...
public class PatientInformationDoctor {
    ...
    void onUpdate(){
        String query = ("UPDATE patient SET Prescription = ?, PathologyTests = ? WHERE ID = ?");

        Update.onUpdateDoctor(query, jPatientID.getText(),jPrescription.getText(), jPathologyTests.getText());
    }
}
